...
import com.eyecall.connection.DefaultProtocolHandler;
...
public class PushRegistration {
	...
	private static final Logger logger = LoggerFactory.getLogger(PushRegistration.class);
    	...
    	if(!prefs.contains(ProtocolField.VOLUNTEER_ID.getName())){
	    	...
	    	new AsyncTask<Void, Void, String>() {
	    		...
	    		protected String doInBackground(Void... params) {
    				try {    					
    					//register at GCM
    					String key = gcm.register(SENDER_KEY);
    					//send registry key to server
    					Connection c = new Connection(Constants.SERVER_URL, Constants.SERVER_PORT, new VolunteerProtocolHandler(), VolunteerState.INITIALISATION);
    					c.init(false);
    					c.send(new Message(ProtocolName.REGISTER).add(ProtocolField.VOLUNTEER_ID, key));
    				} catch (IOException e) {
